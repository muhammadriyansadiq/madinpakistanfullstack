...
async function name(){
    try{
      const getcategorydata = await axios.get(`${import.meta.env.VITE_API_KEY}/api/brand`);
   

    console.log("databrand",getcategorydata.data.data)
setbranddata(getcategorydata.data.data)
    }
    catch(err){
...
const handleDeleteItem = async (id) => {
    try {
      setloading(true);
      const response = await axios.delete(`${import.meta.env.VITE_API_KEY}/api/brand/${id}`);
                        ...
                        <Link to={'/admin/brand/create'}>
                            <div className='cursor-pointer'>
                                <button className='bg-green-700 border-[1px] text-white p-2 rounded-md hover:bg-white hover:text-green-700 hover:border-green-700'>
                                   <span className=" text-[17px] font-semibold">AddBrand</span> 
                                </button>
                            </div>
                        </Link>
                    </div>
                                ...
                                <table className="w-full border-[2px] bg-white  border-gray-300">
                                    <thead>
                                        <tr className="bg-green-700 text-white leading-10 border-gray-500 border-[2px]">
                                            ...
                                            <th className="px-4 py-2 border-[1px] border-gray-400">BrandName</th>
                                        ...
                                        {branddata.map((row, index) => (
                                            <tr key={index} className="border border-gray-300">
                                                ...
                                                <td className=" py-2 border border-gray-300 text-center">
                                                    ...
                                                    <Link to={`/admin/brand/${row._id}`} className="bg-white rounded-lg border-[1px] border-green-700 text-[19px] p-2 text-green-700 hover:bg-green-700 hover:text-white"><FaRegEdit /></Link>
...
<div className=' w-full flex justify-center '>
           ...
           <div className='w-full flex justify-center mt-9 allorders'>
                <div className='w-full border-[2px] rounded-md mb-3'>
                    <div className='flex justify-between p-5 border-b-[1px]'>
                        <div className='font-sans font-medium text-green-700 text-[18px] '>Banner List</div>
                        <Link to={'/admin/brand/create'}>
                            <div className='cursor-pointer'>
                                <button className='bg-green-700 border-[1px] text-white p-2 rounded-md hover:bg-white hover:text-green-700 hover:border-green-700'>
                                   <span className=" text-[17px] font-semibold">AddBrand</span> 
                    ...
                    <div className='flex p-5 justify-center text-[18px] bg-white'>
                       No User Found Please Create Brand
                    </div>
