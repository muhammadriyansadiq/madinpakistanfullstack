  ...
  const fetchBanner = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_KEY}/api/shipping`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_API_KEY}/api/shipping/${id}`,
        formData
      );
              ...
              <form action="" onSubmit={handleSubmit}>
                ...
                <div className="p-5  w-full ">
                  <div className="">
                    <button
                      type="submit"
                      className=" bg-green-700 py-2 border-[2px] font-semibold border-green-700 hover:bg-white hover:text-green-700 px-5 text-white rounded-md "
                    >
